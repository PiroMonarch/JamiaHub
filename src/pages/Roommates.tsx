...
export default function Roommates() {
  ...
  useEffect(() => {
    fetch('/api/roommates').then(res => res.json()).then(setRoommates);
    fetch('/api/saved').then(res => res.json()).then((data: any[]) => {
  ...
  const toggleSave = async (id: number) => {
    const res = await fetch('/api/saved', {
      ...
      body: JSON.stringify({ itemId: id, type: 'roommate' })
    });
    ...
    <div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display antialiased min-h-screen pb-28">
      <div className="fixed top-0 left-0 right-0 z-50 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 transition-all duration-300">
        ...
        <div className="px-5 pb-4 overflow-x-auto no-scrollbar flex gap-2.5">
          <button className="bg-primary text-white px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap shadow-sm shadow-primary/20 transition-all active:scale-95">All Profiles</button>
          ...
          <button className="bg-white dark:bg-gray-800 text-slate-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 px-5 py-2 rounded-full text-sm font-medium whitespace-nowrap active:bg-gray-50 dark:active:bg-gray-700 transition-all active:scale-95">Budget &lt; ₹5k</button>
        </div>
      </div>
