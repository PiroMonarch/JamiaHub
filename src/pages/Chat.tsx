...
export default function Chat() {
      ...
      try {
        ...
        if (data.type === 'typing') {
          ...
          const newMessage: Message = {
            ...
            status: 'read' // Auto-read for demo
          };
          setMessages(prev => [...prev, newMessage]);
          setIsTyping(false);
          
          // Send read receipt back immediately for demo purposes
          if (ws.current?.readyState === WebSocket.OPEN) {
            ws.current.send(JSON.stringify({ type: 'read_receipt', messageId: newMessage.id }));
          }
  ...
  const sendMessage = () => {
    ...
    if (ws.current?.readyState === WebSocket.OPEN) {
      ws.current.send(JSON.stringify({ type: 'message', text: inputText }));
    }
  };
